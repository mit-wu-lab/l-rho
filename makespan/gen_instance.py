...
def main_gen_breakdowns(breakdown_configs, 
    ...
    breakdown_full_name = instance_name + f'-{breakdown_suffix}' if breakdown_suffix else instance_name
    breakdown_dir = os.path.join(data_dir, f'machine_breakdown/breakdown_data/{breakdown_full_name}')
    os.makedirs(breakdown_dir, exist_ok=True)

    for idx in range(n_data):
        if idx % 50 == 0:
            print(f'Generating breakdown events {idx} / {n_data}')

        file_path = os.path.join(dir_path, f'data_{idx}.pkl')

        machine_breakdown_path = os.path.join(breakdown_dir, f'breakdown_{idx}.pkl')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_data', type=int, default=600, help='number of FJSP instances to generate')

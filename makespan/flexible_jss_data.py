...
def get_tasks_features(tasks_dict, jobs_data_sel, jobs_start_time, 
    ...
    for job_id, job in jobs_data_sel.items():
        for task_id, task in job.items():
            ...
            if is_overlap:
                alt_machine = overlapping_jobs_solution_all[(job_id, task_id)][1]
                alt_duration = overlapping_jobs_solution_all[(job_id, task_id)][3]
            else:
                alt_machine, alt_duration = -1, -1
                
                # find the machine with the smallest duration
                alt_durs = [task[alt][0] for alt in task]
                alt_machines = [task[alt][1] for alt in task]
                alt_machine = alt_machines[np.argmin(alt_durs)]
                alt_duration = np.min(alt_durs)
